...
if ( ! class_exists( 'WP_Package_Updater' ) ) {
	...
	class WP_Package_Updater {
		...
		public function __construct(
			...
			$this->use_license       = $use_license; //No lo usamos

			//Datos de la licencia del producto si aplica
			$this->RC_license_key = $license_key;
			$this->RC_domain      = $domain;
			$this->RC_product     = $product_id;
			...
			//Datos de RC
			$metadata_url .= '&RC-license-key=' . rawurlencode( $this->RC_license_key );
			$metadata_url .= '&RC-domain=' . rawurlencode( $this->RC_domain );
			$metadata_url .= '&RC-product=' . rawurlencode( $this->RC_product );
		...
		protected function handle_license_errors( $license_data, $query_type = null ) {
			...
			if (
				...
				$license_data->clear_key = true;
				$license_data->message   = __( 'The provided license key does not appear to be valid.', 'wp-package-updater' );
